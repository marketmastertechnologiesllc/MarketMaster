...
function Dashboard() {
  ...
  const [selectedAccountData, setSelectedAccountData] = React.useState(null);
    ...
    async function fetchAccounts() {
      try {
        ...
        if (accountsData.length > 0) {
          ...
          setSelectedAccountData(accountsData[0]);
        }
      } catch (error) {
  ...
  // Fetch account details and info when account changes
  React.useEffect(() => {
    ...
    async function fetchAccountData() {
      try {
        ...
        const accountData = accounts.find(acc => acc.accountId === selectedAccount);
        setSelectedAccountData(accountData);

        // Fetch all data in parallel
        const [detailsRes, infoRes, tradesRes, historyRes] = await Promise.all([
          api.get(`/account/accounts/${selectedAccount}`),
          api.get(`/account/accountInfo/${selectedAccount}`),
          api.get(`/trade/${selectedAccount}?page=1&pagecount=100&sort=&type=`),
          api.get(`/trade/history/${selectedAccount}?page=1&pagecount=100&sort=&type=`)
        ]);
  ...
  return (
    <div className="w-full text-[#E9D8C8] pb-[100px] space-y-2 sm:space-y-2 lg:px-2">
      {/* Header Section */}
      <div className="bg-[#0B1220] border border-[#11B3AE] rounded-xl p-2 sm:p-4 shadow-[0_0_16px_rgba(17,179,174,0.3)]">
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-2 sm:gap-3">
          <div className="w-full xl:w-auto">
            <h1 className="text-md sm:text-lg lg:text-xl font-bold text-white mb-1 sm:mb-2">Trading Dashboard</h1>
            <p className="text-xs sm:text-sm text-gray-400">
              Comprehensive overview of {selectedAccountData ? `${selectedAccountData.name || selectedAccountData.accountName}(${selectedAccountData.login})` : 'account'}
            </p>
          </div>
            ...
            <FormControl size="small" sx={{
              ...
              <Select
                ...
                renderValue={(value) => {
                  ...
                  return selectedAcc ? `${selectedAcc.name || selectedAcc.accountName} (${selectedAcc.login})` : 'Select Account';
                }}
                MenuProps={{
                  PaperProps: {
                    sx: {
              ...
              >
                {accounts.map(account => (
                  <MenuItem key={account.accountId} value={account.accountId}>
                    {account.name || account.accountName} ({account.login})
                  </MenuItem>
                ))}
              </Select>
            </FormControl>
            ...
            <div className="space-y-3 sm:space-y-4 flex-1 flex flex-col justify-center">
              <div className="grid grid-cols-2 gap-2 sm:gap-4">
                <div className="bg-[#0B1220] border border-[#11B3AE] rounded-lg p-2 sm:p-3">
                  ...
                  <div className="text-sm sm:text-md font-bold text-white">${Number(dashboardMetrics.totalEquity).toFixed(2)}</div>
                </div>
                <div className="bg-[#0B1220] border border-[#11B3AE] rounded-lg p-2 sm:p-3">
