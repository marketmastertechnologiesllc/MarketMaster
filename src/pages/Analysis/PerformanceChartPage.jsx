...
function PerformanceChartPage() {
    ...
    async function fetchAccountData() {
      try {
        ...
        const [detailsRes, infoRes, tradesRes, historyRes] = await Promise.all([
          ...
          api.get(`/trade/${accountId}?page=1&pagecount=100&sort=&type=`),
          api.get(`/trade/history/${accountId}?page=1&pagecount=100&sort=&type=`)
        ]);
  ...
  return (
    <div className="w-full text-[#E9D8C8] pb-[100px] space-y-2 sm:space-y-2 lg:px-2">
      {/* Header Section */}
      <div className="flex flex-col bg-[#0B1220] border border-[#11B3AE] rounded-xl p-3 sm:p-4 lg:p-6 shadow-[0_0_16px_rgba(17,179,174,0.3)]">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-2 sm:gap-3">
          <div className="w-full lg:w-auto">
            ...
            <h1 className="text-md sm:text-lg lg:text-xl font-bold text-white mb-1 sm:mb-2">Performance Chart</h1>
            <p className="text-xs sm:text-sm text-gray-400">
              Detailed performance analysis for Account ID: {accountId}
            </p>
          </div>
