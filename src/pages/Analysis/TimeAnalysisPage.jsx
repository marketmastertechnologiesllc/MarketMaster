...
function TimeAnalysisPage() {
  ...
  }, [startDate, endDate, tradeHistory]);

  // Calculate dashboard metrics
  React.useEffect(() => {
    if (!accountDetails || !accountInfo || !filteredHistoryData || !openTrades) return;
    ...
    // Calculate profit factor and averages from filtered data
    const winningTrades = filteredHistoryData.filter(trade => parseFloat(trade.profit || 0) > 0);
    const losingTrades = filteredHistoryData.filter(trade => parseFloat(trade.profit || 0) < 0);
  ...
  }, [accountDetails, accountInfo, filteredHistoryData, openTrades]);
  ...
  const filterDataByDateRange = () => {
    if (!startDate || !endDate || !tradeHistory.length) return;

    const filtered = tradeHistory.filter(item => {
      const itemDate = new Date(item.start_time || item.openTimeAsDateTime);
  ...
  const generateChartData = () => {
    // Generate daily P&L data from filtered trade history
    const dailyData = {};
    filteredHistoryData.forEach(trade => {
  ...
  const filteredHistory = selectedSymbol === 'All Symbols' ? filteredHistoryData : filteredHistoryData.filter(trade => trade.symbol === selectedSymbol);
  ...
  return (
    <div className="w-full text-[#E9D8C8] pb-[100px] space-y-2 sm:space-y-2 lg:px-2">
      ...
      <div className="bg-[#0B1220] border border-[#11B3AE] rounded-xl p-3 sm:p-4 lg:p-6 shadow-[0_0_16px_rgba(17,179,174,0.3)]">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-2 sm:gap-3">
          <div className="w-full lg:w-auto">
            ...
            <h1 className="text-md sm:text-lg lg:text-xl font-bold text-white mb-1 sm:mb-2">Time Analysis</h1>
            <p className="text-xs sm:text-sm text-gray-400">
              Time-based analysis for Account ID: {id}
            </p>
            {!isDateModalOpen && startDate && endDate && (
              <div className="mt-2">
                <p className="text-xs sm:text-sm text-[#E9D8C8]">
                ...
                <p className="text-xs text-[#11B3AE]">
      ...
      {/* Show loading or date selection prompt */}
      {isDateModalOpen ? (
        ...
        <>
          ...
          {visibleComponents.monthlyStats && (
            <div className="w-full">
              <MonthlyStats
                tradeHistory={filteredHistoryData}
                selectedAccount={id}
              />
            </div>
          )}
