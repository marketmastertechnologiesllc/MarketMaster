...
export default function HistoryTable({ headers }) {
  ...
  React.useEffect(() => {
    ...
    setPage(config.history.page);
    setRowsPerPage(config.history.pagecount);
    setSort(config.history.sort);

    async function fetchData() {
      try {
        const { page, pagecount, sort, type } = config.history;
        const res = await api.get(
          `/trade/history/${id}?page=${page}&pagecount=${pagecount}&sort=${sort.id}&type=${sort.type}`
        );
        setData(res.data.data);
        setCount(res.data.count);
      } catch (error) {
                  ...
                  <TableRow>
                    ...
                    >
                      <div className="flex flex-col justify-center items-center">
                        <span className="text-[#E9D8C8] text-lg font-medium">No history data found</span>
                        <span className="text-[#E9D8C8] text-sm opacity-70 mt-1">No trading history available</span>
                      </div>
                    </TableCell>
                  </TableRow>
                )}
